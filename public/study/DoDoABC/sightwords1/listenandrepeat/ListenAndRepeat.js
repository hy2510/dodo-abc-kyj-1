...
const setExample = () => {
    ...
    if (quizDataArr[correctCount].Image1 == undefined || quizDataArr[correctCount].Image1 == NaN || quizDataArr[correctCount].Image1 == '') {
        ...
        $(".js-wrapper-img").append(`<img src="${quizDataArr[correctCount].Image1}" />`);
        $(".js-text-question").html(quizDataArr[correctCount].Question).addClass("sentence");
        isQuizWord = false;
    }
...
const displayWord = (data) => {
    ...
    for (let i = 0; i < referenceText.length; i++) {
        ...
        if (referenceText[i] !== spans.eq(i).text() && /^[a-zA-Z0-9]$/.test(referenceText[i])) {
            spans.eq(i).text(referenceText[i]);
        }

        if (referenceText[i] === '.' || referenceText[i] === '-' || referenceText[i] === ' ' ||
                referenceText[i] === ',' || referenceText[i] === '\'' ||
                referenceText[i] === '!' || referenceText[i] === '?') {
...
function displayPhoneme(data) {    
    ...
    $.each(data.words, function (index, word) {
        ...
        $.each(word.phonemes, function (i, phoneme) {
            ...
            const alphabetDiv = $('<div>').addClass('alphabet').text(phoneme.alphabet);
