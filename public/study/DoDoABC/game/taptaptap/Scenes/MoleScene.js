...
import PopupNext from "../Objects/Common/PopupNext.js";
...
export default class MoleScene extends Phaser.Scene {
  ...
  preload() {
    ...
    // 예제 데이터 넣어주기
    this.quizData.map((data, i) => {
      this.exampleArr.push(data.Question);
      const obj = {
        key: `questionSound${i + 1}`,
        audio: data.Sound1,
      };

      tempSoundArr.push(obj);
    });

    tempSoundArr.map((data, i) => {
      this.load.audio(data.key, data.audio);
    });
  ...
  create() {
    ...
    this.quizData.map((data, i) => {
      this.sound.add(`questionSound${i + 1}`);
    });
  ...
  createRandomQuestion() {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (
        array.indexOf(this.exampleArr[randomNum]) == -1 &&
        this.exampleArr[randomNum] != this.correctText
      ) {
  ...
  moleUp() {
    ...
    const promises = this.numArr.map((position, index) => {
      return new Promise(() => {
        ...
        this.moles[position].texture.style._font =
          "42px Sandoll Gyeokdonggulim";
  ...
  playQuestionSound() {
    ...
    this.correctText = this.quizData[this.correctCount].Question;
    const sound = `questionSound${this.correctCount + 1}`;
    this.questionSound = this.sound.add(sound);

    this.questionSound.play(audioConfig);
